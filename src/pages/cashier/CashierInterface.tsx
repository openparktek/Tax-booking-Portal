...
export default function CashierInterface() {
  ...
  const processPayment = async () => {
    ...
    if (paymentMethod === "cash") {
      ...
      if (!received || received < currentBooking.fare) {
    ...
    try {
      ...
      const change = paymentMethod === "cash" 
        ? parseFloat(amountReceived) - currentBooking.fare 
        : 0;
      ...
      setLastTransaction({
        ...
        cashierName: "Cashier 1", // In production, get from logged-in user
      });
        ...
        <div className="flex items-center justify-between max-w-7xl mx-auto">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-gradient-to-br from-blue-600 to-green-600 rounded-full flex items-center justify-center">
              <img src="/openpark-logo.png" alt="OpenPark" className="w-10 h-10 rounded-full object-cover" />
            </div>
            <div>
              <h1 className="text-gray-900">Cashier Terminal</h1>
              <p className="text-sm text-gray-600">Payment Collection & Receipt Printing</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
              ...
              {paymentMethod === "cash" ? (
                <div>
                  ...
                  <Input
                    ...
                    className="mt-1.5"
                  />
                  {amountReceived && parseFloat(amountReceived) >= currentBooking.fare && (
                    <p className="text-sm text-green-600 mt-2">
                      Change: {formatCurrency(parseFloat(amountReceived) - currentBooking.fare)}
                    </p>
                  )}
                </div>
              ) : (
